...
(client_id, cc_type, cc_number, cc_expiration_date, ck_routing_number, ck_account_number, ck_number, xaction_status, date, amount, program_id, program_cost, number_of_payments, equal_payments_of, notes, pc_id, date_sold, client_f_name, client_l_name) VALUES ('$client_id', '$cc_type', '$cc_number', '$cc_expiration_date', '$ck_routing_number', '$ck_account_number', '$ck_number', '$xaction_status', '$charge_date', '$amount_to_charge', '$program_index', '$total_program_cost', '$number_of_payments', '$equal_payments_of', '$financial_notes', '$pc_id', '$date_sold', '$f_name', '$l_name')";
...
//**************end Sergiu********************************************************
  ...
  $client_mail_body = "$intro_letter"."<br>Please confirm your received this email by following this link: <a href='$email_receipt_url'>$email_receipt_url</a>  or you can copy this link to your favorite browser.<br><br><br> ";
...
//mail("sergiu.varga@gmail.com", $user_subject, $msg, $mailheaders);
        ...
        $sql = "INSERT INTO email_receipt (id , id_user , client_email , mail_body ,  code , status , DATA )";
        $sql.= " VALUES ( '' , $user_id , '$recipient' , '$client_mail_body' , '$code' , 0 , NOW()  )";
        $results = mysql_query($sql);

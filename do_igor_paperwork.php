...
//include standard includes
  ...
  $address = $descriptions['address_physical'];
  $city = $descriptions['physical_city'];
  $state = $descriptions['physical_state'];
  $zip = $descriptions['physical_zip'];  
...
/* --------------------------------- SEND SPREADSHEET PROMISSORY NOTE TO MEREDITH ---------------------------*/

                                $promissary_subject="****Promissary Note for $f_name $l_name";
                                $promissary_message=$promissary_note;
                                //mail($cc_to_meredith, $no_email_subject, $no_email_message, $mailheaders);
                                $m1= new Mail; // create the mail
                                $m1->From( $headers['from'] );
                                $m1->To( "$cc_to_meredith" );
                                //$m1->To( "sergiu.varga@gmail.com" ); 
                                $m1->ReplyTo ( $headers['replyto']);
                                $m1->Subject( "$promissary_subject" );
                                //$m1->Subject( "Affiliate no email" );
                                $m1->Content_type($headers['contenttype']);
                                //$m1->Cc( "$cc_to_larry" );
                                $m1->Priority(3) ;        // set the priority to Normal
                                $m1->Body("$promissary_message");
                                $m1->Send();

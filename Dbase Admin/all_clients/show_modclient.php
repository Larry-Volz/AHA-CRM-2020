...
echo "Action at Appt: $actionPrintBlock";
...
echo "Client Flags: $flagsPrintBlock";
?>
		</tr>
	</td>
</table>
...
?>
			...
			<font size="2">
				...
				Call After: <select size="1" name="call_before">
			...
			</select>and/or Before:<select size="1" name="call_after">
			<option selected>Start</option>
			<option>6:00 AM</option>
			<option>7:00 AM</option>
			<option>8:00 AM</option>
...
//################################# set up assigned to drop-down ################################

//Start $recordMgrSelectBox
$assignedToSelectBox="<select name = \"record_manager\">";

// establish query to get record manager's' names and id#'s
$assignedToQuery="SELECT f_name, l_name, id 
FROM auth_users
WHERE member_type !='therapist'
ORDER BY f_name";

//run query
$resultola=mysql_query($assignedToQuery);

	//get info about record manager
	WHILE ($row=mysql_fetch_array($resultola)){
	  
	  $assignedTo_f_name=$row['f_name'];
	  $assignedTo_l_name=$row['l_name'];
	  $assignedTo_id=$row['id'];  

	  
	  	//make the options list
	  	$assignedToSelectBox .= "<option value = \"".$assignedTo_id."\"";
	  	
	  		//check if selected
	  		if ($assigned_to == $assignedTo_id){//if this choice is one of those selected...
			  	
							//	$msg .= "\$choice_id = $choice_id and \$id = $id -> Match!<br><br>"; //testing????????			  
	  						//if yes then list as selected
						  	$assignedToSelectBox .= " selected";
			  			  
			}//end of if statement 
	  
	  //add clients names	
	  $assignedToSelectBox.= ">".$assignedTo_f_name."&nbsp;".$assignedTo_l_name."</option>";
	  
	}//end while statement

//put final tag on select box
$assignedToSelectBox .= "</select>";

echo "$assignedToSelectBox";
...
<br>
First Appointment:</font><font face="Arial"><font size="2">
			
				</font></strong>

			<font size="3">

				<select name="first_appointment_month" onchange="populateDays(this)" size="1">
					<option value="">Month</option>
					<option value="0">January</option>
					<option value="1">February</option>
					<option value="2">March</option>
				...
				<select name="first_appointment_day" onchange="format_sales_contact_next();" size="1">
					<option>Day</option>
				</select></font> <font size="3">
				
				<select name="first_appointment_year" onchange="format_sales_contact_next();" size="1">
					<option value="">Year</option>
					<option>2006</option>
					<option value=2007 selected>2007</option>
					<option value=2009>2009</option>
				...
				</select></font></font><font face="Arial" size="2">
			
				<br>
			Scheduled with Affiliate:<select size="1" name="affiliate_id">
			<option value="0">Choose Affiliate</option>
			</select>
			<br>
			<input type="checkbox" name="paperwork_received" value="ON">Paperwork Received</font></p>

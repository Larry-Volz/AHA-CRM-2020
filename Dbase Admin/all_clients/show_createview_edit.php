...
//------------------------------ END LOG ENTRY --------------------------------------------------
 ...
 if ($_POST['edit']!="1")
 {
   ...
   $sql = "select * from $view_sql_table where id=$_GET[idview] ";
   $cdb1->select($sql);
   $view = $cdb1->FetchArray();

   foreach  ( $view AS $key=>$value ) $_POST[$key]=$value;

   //generate group and sort field values from database
   for ($i=1 ;$i<=3;$i++)
   {
      $_POST["group$i"]=$view["group_by$i"];
      $_POST["sort$i"]=$view["sort_by$i"];
   }

   $_POST['name_of_view'] = UnFormatUserInputText($view["viewName"]);
   $_POST['used_by']=$view["view_perms"];
  // $_POST['and_or2'] =$view['all_or_filtered'];


   //generate hidden column name
   $arr1=array();
   ...
   //generate  column name alias
   $arr1=array();
   $arr1= explode(";",$view['all_or_some_records_alias']);
   $i=0;
   foreach ($arr1 as $key=>$value )
   if (!empty($value))
   {
   ...
   $_POST["fields"]=$view['all_or_some_records'];

   //generate  filter fields
   for($i=1;$i<=4;$i++)
   {
      $_POST["filter{$i}"] = $view["filter$i"];
      $_POST["operator{$i}"]= $view["operator{$i}"];
      $_POST["filter{$i}_operand"]=$view["filter{$i}_operand"];
      $_POST["f{$i}_date"]=$view["filter{$i}_date"];
      $_POST["f{$i}_date_input"]=$view["filter{$i}_date_input"] ;
      $_POST["f{$i}_date_input_text"]= $view["filter{$i}_date_input_text"] ;
   }
   ...
   //generate subtotal vals
   $arr1=array();
   $arr1= explode(";",$view["subtotal"]);
   $i=1;
   foreach($total_arr_fields as $key=>$value)
   {
      if ( in_array($key, $arr1) ) $_POST["subtotal$i"]=$key;
      $i++;
   }

   //generate total vals
   $arr1=array();
   $arr1= explode(";",$view["total"]);
   $i=1;
   foreach($total_arr_fields as $key=>$value)
   {
      if ( in_array($key, $arr1) ) $_POST["total$i"]=$key;
      $i++;
   }
...
<p><font size="6">All Clients Database:&nbsp; Edit View</font></p>
...
<?
if ($_POST['edit']=="1") $idv=$_POST['idview'];
else                     $idv=$_GET['idview'];
?>
<INPUT TYPE="HIDDEN"  name="idview" value="<?=$idv?>">
        ...
        <table border="0" width="100%" id="table6">
                ...
                <tr>
                        ...
                        <td><b>Name of View:</b> <input type="text" name="name_of_view" value='<?=$_POST['name_of_view']?>' size="39"?>' size="39"></td>
...
<?
        ...
        {
        $arr_inter_field = explode(".",$_POST['intermediate_field']);

        if (count($arr_inter_field)>1)       $_POST['intermediate_field'] =   $arr_inter_field[1];

        }
        $ifv ="<option value='$_POST[intermediate_field]'>".$_POST['intermediate_field']."</option>";
...
?>       <br /><br />
        <b>Intermediate screen:</b> <input type="radio" name="intermediate_screen" value="0" <?=$no_radio?> > No&nbsp;&nbsp;&nbsp;<input type="radio" name="intermediate_screen" value="1" <?=$yes_radio?>> Yes

   <br /><b>Choose Intermediate Field:</b>    <SELECT  name='intermediate_field'  ><?=$ifv.$select1?></SELECT> <br /><br />
...
<SELECT  name='client' MULTIPLE style='width: 200px; height: 200px;'><?=$field_sel?></SELECT>
        ...
        <p><b>Filtering:</b><br>
        <i>On edit filtering fields must be re-entered</i><br><br>
        <?
        ...
        <input type="radio" value="" <?=$check1?> checked name="all_or_filtered">Choose all records<br>
...
if     ( $fields_arr[$value]== "sales_category" )
      ...
      {
         $view_sql.= ", CONCAT( $modified_by_table.f_name , ' ', $modified_by_table.l_name ) as '".$_POST["$ind"]."'";
...
//  echo $join;
  ...
  foreach($_POST as $key=>$value)
  {
          ...
          $global_vars[$key] = $value;
            ...
            $sql="update all_clients_views
                    set
                       viewName='".FormatUserInputText($name_of_view)."',
                       view_perms='$used_by',
                       queryString='".addslashes($view_sql)."',
                       group_by1='$group1',
                       group_by2='$group2',
                       group_by3='$group3',
                       sort_by1='$sort1',
                       sort_by2='$sort2',
                       sort_by3='$sort3',
                       all_or_some_records='".FormatUserInputText($all_or_some_records)."',
                       all_or_some_records_alias='".FormatUserInputText($all_or_some_records_alias)."',
                       filter1='$filter1',
                       operator1='$operator1',
                       filter1_operand='$filter1_operand',
                       filter1_date= '$f1_date',
                       filter1_date_input='$f1_date_input',
                       filter1_date_input_text= '$f1_date_input_text',
                       and_or1='$and_or1',
                       filter2='$filter2',
                       operator2='$operator2',
                       filter2_operand='$filter2_operand',
                       filter2_date= '$f2_date',
                       filter2_date_input='$f2_date_input',
                       filter2_date_input_text='$f2_date_input_text',
                       and_or2='$and_or2',
                       filter3='$filter3',
                       operator3='$operator3',
                       filter3_operand='$filter3_operand',
                       filter3_date='$f3_date',
                       filter3_date_input='$f3_date_input',
                       filter3_date_input_text='$f3_date_input_text',
                       and_or3='$and_or3',
                       filter4='$filter4',
                       operator4='$operator4',
                       filter4_operand='$filter4_operand',
                       filter4_date='$f4_date' ,
                       filter4_date_input='$f4_date_input',
                       filter4_date_input_text='$f4_date_input_text',
                       subtotal='$subtotal_string',
                       total='$total_string',
                       user_id='".$_SESSION['user']."',
                       all_or_filtered='$all_or_filtered',
                       asc_or_desc1='$asc_or_desc1',
                       asc_or_desc2='$asc_or_desc2',
                       asc_or_desc3='$asc_or_desc3',
                       asc_or_desc_groupby1='$asc_or_desc_groupby1',
                       asc_or_desc_groupby2='$asc_or_desc_groupby2',
                       asc_or_desc_groupby3='$asc_or_desc_groupby3',
                       intermediate_screen = '$intermediate_screen',
                       intermediate_field  = '$intermediate_field'
                    where id=$_GET[idview] ";
...
{
   message_box('View succesfully updated!');

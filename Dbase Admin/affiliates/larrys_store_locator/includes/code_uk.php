...
$query = "SELECT longt,lat,place FROM `data_uk` WHERE postcode='" . $postcode4 . "' OR postcode='" . $postcode3 . "' OR postcode='" . $postcode2 . "' OR postcode='" . $postcode1 . "' ORDER BY length(postcode) desc LIMIT 1";
...
IF( (isset($x1)) AND (isset($y1)) )
{
	...
	$query = "SELECT id,address,telephone,area,longt,lat FROM data_uk, stores WHERE  postcode = post_code and (POW((69.1*(longt-" .  $x1. ")*cos(" . $x1 . "/57.3)),\"2\")+POW((69.1*(lat-" . $y1 . ")),\"2\"))<(" . $_POST['store_radius'] . "*" . $_POST['store_radius'] . ")";
	...
	while ($info = @mysql_fetch_array($query_result))
	{
		...
		IF(calculate_distance($x1,$y1,$info['longt'],$info['lat']) <= $_POST['store_radius'])
		{
			...
			$stores[$counter]['distance'] = calculate_distance($x1,$y1,$info['longt'],$info['lat']);
	
			$distance[calculate_distance($x1,$y1,$info['longt'],$info['lat'])][] = $counter;
		}	
	}
}
...
ELSE
{
	...
	foreach ($distance as $key9 => $value9) 
	{	
		foreach ($value9 as $key7 => $value7) 
		{	
	...
	$page->SetParameter ('RADIUS', $_POST['store_radius']);
